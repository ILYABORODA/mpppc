...
if ( ! defined( 'EPTA_PLUGIN_CURRENT_VERSION' ) ) {
	define( 'EPTA_PLUGIN_CURRENT_VERSION', '1.6.6' );
}
...
if ( ! class_exists( 'EventPageTemplatesAddon' ) ) {
	class EventPageTemplatesAddon {
		...
		public function __construct() {
			 ...
			 add_action( 'plugin_row_meta', [ $this, 'eptaaddMetaLinks' ], 10, 2 );
			if ( is_admin() ) {
		...
		function epta_submenu_order(){
			global $submenu;
				$newSubmenu = [];
				foreach ($submenu as $menuName => $menuItems) {
					if ('cool-plugins-events-addon' === $menuName) {
						 $test = $menuItems[0];
						if($test[0]=='Single Event Template'){
								$total = count($menuItems);
							for($i=1;$i<$total;$i++){
								$newSubmenu[$i] = $menuItems[$i];
							}
							$newSubmenu[$total]=$menuItems[0];
		...
		public function eptaaddMetaLinks( $links, $file){
			if ( strpos( $file, basename(__FILE__) )) {
				...
				$eptavideourl = 'https://eventscalendaraddons.com/go/epta-video-tutorial/?utm_source=epta_plugin&utm_medium=inside&utm_campaign=video_tutorial&utm_content=plugins_list';
		...
		*/
		...
		public function epta_post_type() {
			$labels = array(
				...
				'all_items'             => __( 'Single Event Template', 'tecspb2' ),
		...
		*/
		...
		function add_pro_button($which){
			if($which == 'top')
				{	
					$epta_get_post_type = $this->epta_get_post_type_page();
					if ($epta_get_post_type != 'epta') return false;		
			?>
