...
// Allowed HTML tags for heading substitutions (h3 and h4 are used by default)
function r34ics_allowed_heading_tags() {
	return array('h2', 'h3', 'h4', 'h5', 'h6', 'p', 'div');
}
...
function r34ics_has_desc($args, $event) {
	...
	// Assess settings and presence of content to determine whether or not there is a description to display
		...
		$has_desc =
			...
			((!empty($event['multiday'])) && ($args['view'] != 'list' || $args['toggle'] == 'lightbox'));
	}
...
function r34ics_system_report($echo=true) {
	...
	$report = array(
		...
		'Active Theme' => $theme->name . ' v.' . $theme->version . ' (' . pathinfo($theme->template_dir, PATHINFO_BASENAME) . ')',
