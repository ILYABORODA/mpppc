...
class R34ICS {
	...
	protected $shortcode_defaults_new_10_6 = array(
		...
		'eventdesc' => true,
		'hiderecurrence' => true,
		'limitdays' => array('list' => 365, 'week' => 21, 'default' => 455),
		'location' => true,
		'organizer' => true,
		'pastdays' => array('list' => 0, 'week' => 7, 'default' => 90),
		'showendtimes' => true,
		'stickymonths' => true,
		'title' => false,
	);
	...
	public function admin_page() {
		...
		elseif (isset($_POST['r34ics-url-tester-nonce']) && wp_verify_nonce($_POST['r34ics-url-tester-nonce'],'r34ics')) {
			...
			if ($url_to_test = filter_input(INPUT_POST, 'url_to_test', FILTER_SANITIZE_URL)) {
				$this->debug = 2;
				$url_tester_result = method_exists($this, '_url_get_contents') ? $this->_url_get_contents($url_to_test) : false;
				$url_tester_debug = $this->debug_messages;
			}
	...
	public function enqueue_scripts() {
		if ($this->scripts_enqueued) { return; }
	
		// ICS Calendar JS
		wp_enqueue_script('ics-calendar');
		wp_localize_script('ics-calendar', 'r34ics_ajax_obj', array(
	...
	public function event_description_html($args, $event, $classes=array(), $has_desc=null) {
		...
		if (!empty($has_desc) || !empty($descloc_content)) {
			...
			echo '<div class="' . esc_attr(implode(' ', $descloc_class)) . '">';

			// Date(s) -- lightbox only, unless multi-day
			if ($args['toggle'] === 'lightbox' || !empty($event['multiday'])) {
			...
			if (in_array('hover_block', $descloc_class) || $args['toggle'] === 'lightbox' || !empty($event['multiday'])) {
		...
		// Deprecated -- incorrect filter name -- @todo Add deprecation notice
	...
	public function event_label_html($args, $event, $classes=array()) {
		...
		echo '<span class="' . esc_attr(implode(' ', $title_class)) . '">';
		...
		// Deprecated -- incorrect filter name -- @todo Add deprecation notice
		$title_content = apply_filters('r3417_event_label_html_filter', $title_content, $args, $event, $classes);
	...
	public function event_sublabel_html($args, $event, $classes=array()) {
		...
		// Deprecated -- incorrect filter name -- @todo Add deprecation notice
		$sublabel_content = apply_filters('r3417_event_sublabel_html_filter', $sublabel_content, $args, $event, $classes);
	...
	public function shortcode($atts) {
		...
		$args = array(
			...
			'guid_static' => $guid,
			'htmltagdate' => (
				in_array($htmltagdate, r34ics_allowed_heading_tags())
					? $htmltagdate
					: 'h4'
			),
			'htmltagmonth' => (
				in_array($htmltagmonth, r34ics_allowed_heading_tags())
					? $htmltagmonth
					: 'h3'
			),
			'htmltagtitle' => (
				in_array($htmltagtitle, r34ics_allowed_heading_tags())
					? $htmltagtitle
					: 'h2'
			),
			...
			'paginationposition' => (
				in_array($paginationposition, array('above','below','both'))
					? $paginationposition
					: (r34ics_boolean_check($pagination) ? 'below' : null)
				),
			'pastdays' => intval($pastdays),
			'reload' => (
				(intval($reload) && $reload > 1)
					? intval($reload)
					: (r34ics_boolean_check($reload) || intval($debug) >= 2)
				),
	...
	protected function _url_get_contents($url, $method='', $recursion=false, $curlopts=null, $use_transients=false, $basicauth=false, $skip_domain_errors=false) {
		...
		if (defined('CURLVERSION_NOW') && function_exists('curl_exec') && (empty($method) || $method == 'curl')) { 
			...
			$http_headers = array();
			// Set language to match site configuration
			if ($locale = substr(get_locale(),0,2)) {
				$http_headers[] = 'Accept-Language: ' . $locale;
			}
			// Add HTTP headers
