...
function r34ics_init() {
	...
	*/
	...
	if (jQuery('.ics-calendar-list-pagination').length > 0) {
		jQuery('.ics-calendar.layout-list .ics-calendar-paginate').on('click', function() {
			var container = jQuery(this).closest('.ics-calendar.layout-list');
			var current = container.find('.ics-calendar-list-pagination:visible');
		...
		jQuery('.ics-calendar.layout-list').each(function() {
			jQuery(this).find('.ics-calendar-list-pagination:not(:first-child)').hide();
			jQuery('.ics-calendar.layout-list .ics-calendar-paginate.prev').hide();
		});
		r34ics_show_hide_headers();
	}
